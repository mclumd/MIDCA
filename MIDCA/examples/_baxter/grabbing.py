...
def MoveArm(point):
    print('point =', point)
    orientation = baxter.getLeftArmOrientation()
    ...
    return baxter.moveLeftArm(point, orientation)    
...
def reaching_callback(data):
    ...
    msg = data.data
#     point = Point()
#     floats = re.findall(r"[-+]?\d*\.\d+|\d+",msg)
#     point.x = float(floats[0])
#     point.y = float(floats[1])
#     point.z = float(floats[2])-0.95
    #ERROR HERE orien = Quarternion()
    ...
    if not 'x' in d and 'y' in d and 'z' in d and 'cmd_id' in d:
        ...
        try:
            point = Point(x = d['x'], y = d['y'], z = (d['z']-0.95))
